...
const { getCertificatesInfoFromPDF } = require('@jonz94/verify-pdf');
...
app.post('/api/verify', upload.single('pdf'), async (req, res) => {
  try {
    ...
    try {
      const verificationResult = verifyPDF(pdfBuffer);
      ...
      // When subfilter/signature dictionary cannot be found, treat it as
      // "no valid signature / signature structure broken" instead of 500.
      if (lower.includes('cannot find subfilter')) {
        verificationErrorMessage = 'No valid digital signature could be found in this PDF. It may be unsigned or the signature structure was removed or corrupted after signing.';
      } else {
        throw e;
      }
    }
    ...
    if (!verificationErrorMessage) {
      try {
        const rawCerts = getCertificatesInfoFromPDF(pdfBuffer) || [];
        // rawCerts is an array per signature, each containing an array of certificates
        certificatesInfo = Array.isArray(rawCerts)
          ? rawCerts.reduce((all, perSignature) => {
              if (Array.isArray(perSignature)) {
                return all.concat(perSignature);
              }
              return all.concat([perSignature]);
            }, [])
          : [];
      } catch (e) {
        // If certificate extraction fails, continue with what we have
        console.error('Certificate extraction error:', e.message);
      }
    }

    const expectedCommonName = process.env.SIGNER_COMMON_NAME || 'PDF Signer';

    let signedByUs = false;

    if (Array.isArray(certificatesInfo)) {
      for (const cert of certificatesInfo) {
        if (cert && cert.clientCertificate) {
          const issuedTo = cert.issuedTo || {};
          const commonName =
            issuedTo.commonName ||
            issuedTo.CN ||
            issuedTo.cn ||
            issuedTo['Common Name'];
    ...
    res.json({
      ...
      expired,
      signedByUs,
      expectedSignerCommonName: expectedCommonName,
      signatures,
...
Available Endpoints:
  ...
  POST /api/verify                - Verify PDF (multipart)
